...
execute store success score #success aj.i run function animated_java:armor_stand_modify/zzz/19 with storage aj:temp args
$execute unless score #success aj.i matches 1 run return run tellraw @a "The animation $(animation) does not exist."
$tellraw @a "The animation $(animation) has been applied to the rig."
execute if data storage aj:temp {args:{start_animation: 1b}} run function animated_java:armor_stand_modify/zzz/22 with storage aj:temp args
...
execute unless entity @s[type=item_display,tag=aj.rig_root] run return run \
	...
	{'function_path': 'animated_java:armor_stand/animations/exit_sitting/stop'}
tag @s remove aj.armor_stand.animation.exit_sitting.playing
execute store result storage aj:temp frame int 1 run scoreboard players set @s aj.frame 1
function animated_java:armor_stand/animations/exit_sitting/zzz/frames/1